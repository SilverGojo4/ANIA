...
def extract_cgr_features_and_target_for_dl(
    ...
    """
    Extract metadata, features, and target by index range for deep learning (without logger).
    ...
    if target_column not in df.columns:
        raise KeyError(f"Missing '{target_column}' column.")
    ...
    # Check dimensions
    expected_feature_dim = height * width * 11
    if X.shape[1] != expected_feature_dim:
        raise ValueError(
            f"Expected {expected_feature_dim} features (11 x {height} x {width}), "
            f"but got {X.shape[1]}."
        )

    # Reshape to (batch_size, 11, height, width)
    X_tensor = torch.tensor(X.values, dtype=torch.float32)
    X_tensor = X_tensor.view(-1, 11, height, width)
